...
func mustOpenConversation(t testing.TB, m *Memory, convID string, labels []string) *Conversation {
	t.Helper()
	conv, err := m.OpenConversation(convID, labels)
	if err != nil {
		t.Fatalf("OpenConversation(%q): %v", convID, err)
	}
	return conv
}
...
func TestHostOpenRejectsIDContainingSeparator(t *testing.T) {
	...
	if _, err := h.Open("bad:id"); err == nil {
		t.Fatal("expected Open to reject id containing separator")
	}
}
...
func TestConversationAppendAndRecent(t *testing.T) {
	...
	conv := mustOpenConversation(t, m, "device-001", []string{"person:alice"})
...
func TestOpenConversationRejectsConvIDContainingSeparator(t *testing.T) {
	h := newTestHost(t)
	defer h.Close()
	m := mustOpen(t, h, "test")

	badConvID := "device" + string(testSep) + "001"
	if _, err := m.OpenConversation(badConvID, nil); err == nil {
		t.Fatal("expected OpenConversation to reject convID containing separator")
	}
}
...
func TestConversationAutoTimestamp(t *testing.T) {
	...
	conv := mustOpenConversation(t, m, "s1", nil)
...
func TestConversationRevert(t *testing.T) {
	...
	conv := mustOpenConversation(t, m, "device-001", nil)
...
func TestZeroCompressPolicyDisablesAutoCompression(t *testing.T) {
	...
	if count != 40 {
		t.Fatalf("conversation count = %d, want 40 (auto-compress should be disabled)", count)
	}
	...
	if count1h != 0 {
		t.Fatalf("1h bucket should stay empty when auto-compress disabled, got %d", count1h)
	}
}
...
func TestAutoCompressTriggersCompact(t *testing.T) {
	...
	m := mustOpen(t, h, "test")
	ctx := context.Background()
	conv := mustOpenConversation(t, m, "dev1", []string{"person:test"})
...
func TestMemoryCompress(t *testing.T) {
	...
	conv := mustOpenConversation(t, m, "s1", []string{"person:test"})
...
func TestHostDeleteClearsData(t *testing.T) {
	...
	conv2 := mustOpenConversation(t, m2, "dev1", nil)
...
func TestRealisticCompressPipeline(t *testing.T) {
	...
	conv := mustOpenConversation(t, m, "xiaoming-ipad", []string{"person:小明"})
...
func BenchmarkConversationAppend(b *testing.B) {
	...
	conv := mustOpenConversation(b, m, "s1", nil)
	ctx := context.Background()
