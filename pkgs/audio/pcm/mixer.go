...
func (mx *Mixer) CreateTrack(opts ...TrackOption) (Track, *TrackCtrl, error) {
	...
	onTrackCreated := mx.onTrackCreated
	mx.mu.Unlock()

	if onTrackCreated != nil {
		onTrackCreated()
	}
	return tr, tc, nil
}
...
func (mx *Mixer) readFull(p []byte) error {
	...
	for {
		var closedCount int
		var err error
		peak, read, silence, closedCount, err = mx.readFullLocked(p)

		if closedCount > 0 && mx.onTrackClosed != nil {
			onTrackClosed := mx.onTrackClosed
			mx.mu.Unlock()
...
func (mx *Mixer) readFullLocked(p []byte) (peak float32, read, silence bool, closedCount int, err error) {
	...
	// 实时 mixer 语义：只要仍有 track 存在，即使都暂时无数据，也返回静音帧，
	// 而不是阻塞等待，避免单路阻塞拖垮整体输出。
	if !read && mx.head != nil {
		silence = true
	}
