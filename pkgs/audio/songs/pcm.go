...
type RenderOptions struct {
	...
	Volume    float64    // Volume 0.0-1.0; 0 means mute
	Metronome bool       // Include metronome track
	RichSound bool       // true: rich harmonics, false: sine tone
}
...
func (s Song) Render(opts RenderOptions) io.Reader {
	...
	opts.Volume = clamp(opts.Volume, 0.0, 1.0)
	...
	setMixerError := func(err error) {
		if err == nil {
			return
		}
		_ = mixer.CloseWithError(err)
	}
	...
	for i, voice := range voices {
		voiceChunks[i] = voiceToChunk(voice, opts.Format, opts.Volume, opts.RichSound)
	}
	...
	var renderErr error
	joinRenderErr := func(err error) {
		renderErrMu.Lock()
		renderErr = errors.Join(renderErr, err)
		joined := renderErr
		renderErrMu.Unlock()
		setMixerError(joined)
	}

	for i, chunk := range voiceChunks {
		label := voiceLabel(i)
		track, ctrl, err := mixer.CreateTrack(pcm.WithTrackLabel(label))
		if err != nil {
			joinRenderErr(fmt.Errorf("songs/render: create track %q: %w", label, err))
			continue
		}

		wg.Add(1)
		go func(label string, track pcm.Track, ctrl *pcm.TrackCtrl, chunk pcm.Chunk) {
			defer wg.Done()
			defer func() {
				if err := ctrl.CloseWrite(); err != nil {
					joinRenderErr(fmt.Errorf("songs/render: close track %q: %w", label, err))
				}
			}()
			if err := track.Write(chunk); err != nil {
				joinRenderErr(fmt.Errorf("songs/render: write track %q: %w", label, err))
			}
		}(label, track, ctrl, chunk)
	}
	...
	go func() {
		...
		if err := mixer.CloseWrite(); err != nil {
			setMixerError(fmt.Errorf("songs/render: close mixer write: %w", err))
		}
	}()
...
func VoiceToChunk(voice Voice, format pcm.Format, volume float64) pcm.Chunk {
	return voiceToChunk(voice, format, volume, true)
}

func voiceToChunk(voice Voice, format pcm.Format, volume float64, richSound bool) pcm.Chunk {
	...
	for _, n := range voice.Notes {
		...
		noteData := GenerateSineWave(n.Freq, samples, sampleRate)
		for i := 0; i < len(noteData); i += 2 {
			s := int16(noteData[i]) | int16(noteData[i+1])<<8
			scaled := int16(clamp(float64(s)*volume, -32768, 32767))
			binary.LittleEndian.PutUint16(data[offset:], uint16(scaled))
			offset += 2
		}
	}
