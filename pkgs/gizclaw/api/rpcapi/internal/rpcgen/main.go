...
const (
	...
	outDirRel    = "pkgs/gizclaw/api/rpcapi"
...
func run(args []string) error {
	if len(args) == 1 && args[0] == resolveArg {
		return resolveRPC()
	}
	if len(args) > 0 {
		return fmt.Errorf("unknown argument %q", args[0])
	}
	return fmt.Errorf("missing %s", resolveArg)
}

func resolveRPC() error {
	...
	return writeIfChanged(filepath.Join(root, outDirRel, resolvedRPCFile), bundle)
}
...
func (g *generator) bundleSpec() ([]byte, error) {
	...
	bundle := map[string]any{
		"openapi": "3.0.3",
		"info": map[string]any{
			"title":       "RPC API Codegen Bundle",
			"description": "Generated by go run ./internal/rpcgen from api/rpc.json. DO NOT EDIT.",
			"version":     "1.0.0",
		},
