...
// extractArg matches the expected JSON output from the LLM.
...
// GenX implements [Segmentor] using a genx.Generator.
type GenX struct {
	generator string
	mux       *generators.Mux
}
...
func (g *GenX) parseResult(call *genx.FuncCall) (*Result, error) {
	...
	for i, e := range arg.Entities {
		...
		entities[i] = EntityOutput{Label: e.Label, Attrs: attrs}
	}
