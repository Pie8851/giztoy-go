...
// ASR routes recognition requests to registered transformers.
type ASR struct {
...
func NewASRMux() *ASR {
	return &ASR{mux: trie.New[genx.Transformer]()}
}
...
func (m *ASR) Create(ctx context.Context, pattern string) (*ASRSession, error) {
	...
	return &ASRSession{input: inputStream, output: outputStream}, nil
}
...
func (s *ASRSession) Send(data []byte, mimeType string) error {
	chunk := &genx.MessageChunk{Part: &genx.Blob{MIMEType: mimeType, Data: data}}
