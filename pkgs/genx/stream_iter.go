...
func (itr *StreamIter) pull(str Stream) (re error) {
	...
	for {
		...
		case chunk.ToolCall != nil:
			itr.ch <- &ToolCallElement{Role: chunk.Role, Name: chunk.Name, ToolCall: *chunk.ToolCall}
		case chunk.Part != nil:
			key := iterStreamKey{Name: chunk.Name, Role: chunk.Role}
			var (
				data []byte
				cap  int
			)
			...
			if !ok {
				...
				itr.streams[key] = rb
				itr.ch <- &StreamElement{Name: key.Name, Role: key.Role, MIMEType: key.MIMEType, reader: rb}
			}
