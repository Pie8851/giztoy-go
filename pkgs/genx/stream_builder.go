...
func (sb *StreamBuilder) Done(stats Usage) error {
	if err := sb.rb.Add(&StreamEvent{Status: StatusDone, Usage: stats}); err != nil {
		return err
	}
...
func (sb *StreamBuilder) Truncated(stats Usage) error {
	if err := sb.rb.Add(&StreamEvent{Status: StatusTruncated, Usage: stats}); err != nil {
		return err
	}
	return sb.rb.CloseWrite()
}
...
func (sb *StreamBuilder) Blocked(stats Usage, refusal string) error {
	if err := sb.rb.Add(&StreamEvent{Status: StatusBlocked, Usage: stats, Refusal: refusal}); err != nil {
		return err
	}
	return sb.rb.CloseWrite()
}
...
func (sb *StreamBuilder) Unexpected(stats Usage, err error) error {
	if err := sb.rb.Add(&StreamEvent{Status: StatusError, Usage: stats, Error: err}); err != nil {
		return err
	}
	return sb.rb.CloseWrite()
}
