...
func (idx *Index) SearchSegments(ctx context.Context, q SearchQuery) ([]ScoredSegment, error) {
	...
	if idx.embedder != nil && idx.vec != nil && q.Text != "" && idx.vec.Len() > 0 {
		...
		// IMPORTANT: request the full candidate set from the shared vector index
		// and then filter by this Index's segments.
		//
		// Why: multiple personas may share one Vec instance. If we only request
		// a small global topK here, vectors from other personas can crowd out
		// this persona's vectors before filtering, causing false negatives.
		// Using full Len() gives a provably correct persona-local filter.
		topK := idx.vec.Len()
		...
		for _, m := range matches {
			...
			if prev, exists := vecScores[m.ID]; !exists || sim > prev {
				vecScores[m.ID] = sim
			}
		}
	}
