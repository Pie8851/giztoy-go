...
func (m *Memory) CompactBucket(ctx context.Context, bucket recall.Bucket) error {
	...
	// Build a timestamp set for the target bucket so newly compacted segments
	// can keep historical time semantics while still avoiding key collisions.
	existing, err := m.index.BucketSegments(ctx, targetBucket)
	if err != nil {
		return fmt.Errorf("load target bucket %s: %w", targetBucket, err)
	}
	usedTS := make(map[int64]struct{}, len(existing)+len(result.Segments))
	for _, seg := range existing {
		usedTS[seg.Timestamp] = struct{}{}
	}

	// Store the new compacted segments.
	for i, seg := range result.Segments {
		// Keep time semantics: compacted segments represent historical events,
		// so timestamp should stay anchored at lastTS, with minimal monotonic
		// offsets for multi-output compaction.
		segTS := lastTS + int64(i)
		for {
			if _, exists := usedTS[segTS]; !exists {
				break
			}
			segTS++
		}
		...
		newSeg := recall.Segment{
			// Use a fresh ID to avoid colliding with source segment IDs
			// (especially when compacting already-compacted buckets).
			ID:        fmt.Sprintf("%s-cmp-%d", m.id, nowNano()),
			Summary:   seg.Summary,
