...
func keySeparator(sep byte) byte {
	if sep != 0 {
		return sep
	}
	return kv.DefaultSeparator
}

func validateKeySegment(field, value string, sep byte) error {
	if strings.IndexByte(value, keySeparator(sep)) >= 0 {
		return fmt.Errorf(
			"memory: invalid %s %q: contains separator %q",
			field,
			value,
			string(keySeparator(sep)),
		)
	}
	return nil
}
