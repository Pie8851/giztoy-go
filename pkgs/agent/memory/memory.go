...
type Memory struct {
	...
	separator  byte           // key segment separator validation
}

func newMemory(id string, store kv.Store, index *recall.Index, compressor Compressor, policy CompressPolicy, separator byte) *Memory {
	return &Memory{
		id:         id,
		store:      store,
		index:      index,
...
func (m *Memory) OpenConversation(convID string, labels []string) (*Conversation, error) {
	if err := validateKeySegment("convID", convID, m.separator); err != nil {
		return nil, err
	}
	return newConversation(m, convID, labels), nil
}
