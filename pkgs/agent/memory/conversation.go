...
func (c *Conversation) Revert(ctx context.Context) error {
	...
	if latestUserTS > 0 {
		...
		if err := store.Delete(ctx, rk); err != nil {
			return err
		}
	}
...
// recomputePending rebuilds auto-compression counters from current KV state.
// This is used after destructive operations like Revert.
func (c *Conversation) recomputePending(ctx context.Context) error {
	prefix := convMsgPrefix(c.mem.id, c.convID)
	pendingChars := 0
	pendingMsgs := 0
	...
	c.pendingChars = pendingChars
	c.pendingMsgs = pendingMsgs
	return nil
}
