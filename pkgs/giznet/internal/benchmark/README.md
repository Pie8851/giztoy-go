...
```text
pkgs/giznet/internal/benchmark/
├── *_bench_test.go
└── internal/
...
Use `pkgs/giznet/internal/benchmark/run.sh` with system traffic shaping:

- Linux: `tc netem`
- macOS: `pfctl + dnctl`

```bash
./pkgs/giznet/internal/benchmark/run.sh --profile wifi --scale smoke
```
...
./pkgs/giznet/internal/benchmark/run.sh --profile all --scale smoke --no-system-netem --publish-result
```
...
Published benchmark reports default to files under `pkgs/giznet/internal/benchmark/`, and report tables use a short `Key` column (`R01`, `R02`, ...), while full benchmark names are placed in the final `Details` column for readability.