...
PUBLISH_RESULT=0
RESULT_FILE=''
RESULT_DIR='pkgs/giznet/internal/benchmark'
...
usage() {
	cat <<'EOF'
Usage: ./pkgs/giznet/internal/benchmark/run.sh [options]
...
Options:
  ...
  --publish-result             Publish report to pkgs/giznet/internal/benchmark/SMOKE_RESULT.md or FULL_RESULT.md
...
Examples:
  ...
  ./pkgs/giznet/internal/benchmark/run.sh --profile all --scale full --manual --publish-result
  ./pkgs/giznet/internal/benchmark/run.sh --profile all --scale full --manual --bench 'BenchmarkNet_KCPOverNoise_'
EOF
}
...
run_one() {
	...
	local -a cmd
	cmd=(go test ./pkgs/giznet/internal/benchmark -run '^$' -bench "$BENCH_FILTER" -benchmem -count "$COUNT" -benchtime "$BENCHTIME")
...
generate_report() {
	report_file="${out_dir}/report.md"
	{
		...
		echo
		echo "- Script: \`./pkgs/giznet/internal/benchmark/run.sh\`"
		echo "- Scale: \`${SCALE}\`"
...
out_dir="pkgs/giznet/internal/benchmark/results/${timestamp}"
mkdir -p "$out_dir"
