...
type Config struct {
	...
	// ClusterThreshold is the minimum cosine similarity used during
	// reclustering and bucket-ID reuse. Lower = more aggressive grouping.
	// Default: Threshold.
	...
	// MaxSamples bounds the number of stored embeddings retained after each
	// recluster pass. The registry keeps representative samples per bucket and
	// drops low-value/outlier-heavy history to keep memory and recluster cost
	// under control. Default: 128. Set to a negative value to disable pruning.
	MaxSamples int
}
...
func (c *Config) defaults() {
	...
	if c.ClusterThreshold == 0 {
		c.ClusterThreshold = c.Threshold
	}
	if c.MinSamples == 0 {
		c.MinSamples = 2
	}
...
func (r *Registry) Recluster() int {
	...
	r.mu.RLock()
	clusterThreshold := r.cfg.ClusterThreshold
	minSamples := r.cfg.MinSamples
	dim := r.cfg.Dim
	maxSamples := r.cfg.MaxSamples
	r.mu.RUnlock()

	// DBSCAN: eps = 1 - clusterThreshold (cosine distance).
	eps := 1.0 - clusterThreshold
	normed := normalizeEmbeddings(embeddings)
	labels := dbscan(normed, float32(eps), minSamples)

	if maxSamples > 0 && len(embeddings) > maxSamples {
		selection := compactEmbeddings(normed, labels, maxSamples)
		compacted := make([][]float32, 0, len(selection.indices))
		for _, idx := range selection.indices {
			cp := make([]float32, len(embeddings[idx]))
			copy(cp, embeddings[idx])
			compacted = append(compacted, cp)
		}
		if err := r.store.ReplaceAll(compacted); err != nil {
			return 0
		}
		embeddings = compacted
		normed = normalizeEmbeddings(embeddings)
		labels = dbscan(normed, float32(eps), minSamples)
	}

	newBuckets := buildBuckets(normed, labels, dim)
	...
	for i := range newBuckets {
		...
		if bestOldIdx >= 0 && bestSim >= clusterThreshold {
			// Reuse old ID.
...
func buildBuckets(normed [][]float32, labels []int, dim int) []Bucket {
	maxLabel := 0
...
func (r *Registry) SetThreshold(t float32) {
	...
	old := r.cfg.Threshold
	r.cfg.Threshold = t
	if r.cfg.ClusterThreshold == old {
		r.cfg.ClusterThreshold = t
	}
}
