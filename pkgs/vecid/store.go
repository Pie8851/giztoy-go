...
type Store interface {
	...
	// ReplaceAll atomically swaps the store contents with the provided embeddings.
	// Implementations must copy the input data so callers can safely reuse buffers.
	ReplaceAll(embeddings [][]float32) error
...
func (s *MemoryStore) ReplaceAll(embeddings [][]float32) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.data = make([][]float32, len(embeddings))
