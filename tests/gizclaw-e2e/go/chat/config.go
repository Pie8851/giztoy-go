...
const (
	contextConfigDefaultHome = "tests/gizclaw-e2e/testdata/identities"
	contextConfigDefaultName = "peer"
	contextConfigDefaultPath = contextConfigDefaultHome + "/" + contextConfigDefaultName + "/config.yaml"
)
...
func defaultContextConfigPath(configPath string) string {
	...
	candidates := []string{
		filepath.Clean(filepath.Join(configDir, "..", "identities", "peer", "config.yaml")),
		filepath.Clean(filepath.Join(configDir, "..", "..", "testdata", "identities", "peer", "config.yaml")),
...
func defaultClientContextConfigPath() string {
	candidates := []string{
		filepath.Clean(contextConfigDefaultPath),
		filepath.Clean(filepath.Join("..", "..", "testdata", "identities", contextConfigDefaultName, "config.yaml")),
	}
	for _, candidate := range candidates {
		if _, err := os.Stat(candidate); err == nil {
			return candidate
...
func envContextConfigPath(homeEnv, contextEnv, defaultHome, defaultName string) string {
	...
	return filepath.Join(home, name, "config.yaml")
}

func clientContextConfigPath() string {
	return envContextConfigPath(
		"GIZCLAW_E2E_IDENTITIES_HOME",
		"GIZCLAW_E2E_PEER_CONTEXT",
...
func normalizeCipherMode(mode string) string {
	...
	case "", "chacha", "chacha_poly", "chacha20_poly1305":
		return string(gizwebrtc.CipherModeChaChaPoly)
	case "aes", "aes_gcm", "aes_256_gcm", "aes256_gcm":
		return string(gizwebrtc.CipherModeAES256GCM)
	case "plain", "plaintext":
		return string(gizwebrtc.CipherModePlaintext)
	default:
		return mode
	}
}
