...
func TestLoadConfigJSONAndDefaultClientConfig(t *testing.T) {
	...
	contextDir := filepath.Join(dir, "testdata", "identities", "peer")
  ...
  "persona": "short"
	...
	if cfg.Server.CipherMode != string(gizwebrtc.CipherModeChaChaPoly) {
...
func TestNormalizeCipherMode(t *testing.T) {
	tests := map[string]string{
		"":             string(gizwebrtc.CipherModeChaChaPoly),
		"aes-gcm":      string(gizwebrtc.CipherModeAES256GCM),
		"aes_256_gcm":  string(gizwebrtc.CipherModeAES256GCM),
		"plaintext":    string(gizwebrtc.CipherModePlaintext),
		"chacha-poly":  string(gizwebrtc.CipherModeChaChaPoly),
