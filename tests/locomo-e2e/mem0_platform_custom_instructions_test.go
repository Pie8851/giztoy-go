...
func TestLoCoMoMem0PlatformCustomInstructions(t *testing.T) {
	settings := requireLiveSettings(t, liveNeeds{})
	endpoint := os.Getenv("GIZCLAW_LOCOMO_E2E_MEM0_CUSTOM_INSTRUCTIONS_ENDPOINT")
	apiKey := os.Getenv("GIZCLAW_LOCOMO_E2E_MEM0_CUSTOM_INSTRUCTIONS_API_KEY")
	identity := os.Getenv("GIZCLAW_LOCOMO_E2E_MEM0_CUSTOM_INSTRUCTIONS_FINGERPRINT")
