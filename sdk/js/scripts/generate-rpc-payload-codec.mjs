...
const peerProtoURL = new URL(
  "../../../api/proto/rpc/rpc.proto",
  import.meta.url,
);
const payloadProtoDirURL = new URL(
  "../../../api/proto/rpc/payload/",
  import.meta.url,
);
const outputURL = new URL(
  "../gizclaw/generated/rpc/payload-codec.ts",
  import.meta.url,
);
...
if (methods.length === 0) {
  throw new Error(
    "api/proto/rpc/rpc.proto must define RpcMethod entries before generating RPC payload codec.",
  );
}

const requestPayloadMessages = Object.fromEntries(
  methods.map((item) => [item.method, item.request]),
);
const responsePayloadMessages = Object.fromEntries(
  methods.map((item) => [item.method, item.response]),
);
...
function parsePayloadProto(proto) {
  ...
  for (const line of lines) {
    ...
    if (currentEnum != null) {
      if (/^\s*\}/.test(line)) {
        enums[currentEnum.name] = buildEnumDesc(
          currentEnum.name,
          currentEnum.values,
        );
        currentEnum = null;
...
function parseField(line, oneofGroup, messageName) {
  const map =
    /^\s*map<\s*string\s*,\s*([\w.]+)\s*>\s+(\w+)\s*=\s*(\d+)\s*(?:\[([^\]]*)\])?\s*;/.exec(
      line,
    );
  if (map != null) {
    ...
    return {
      ...
      type: "map",
      mapValue: map[1],
      ...(OPTIONAL_MAP_FIELDS.has(`${messageName}.${name}`)
        ? { optional: true }
        : {}),
  ...
  const match =
    /^\s*(optional\s+|repeated\s+)?([\w.]+)\s+(\w+)\s*=\s*(\d+)\s*(?:\[([^\]]*)\])?\s*;/.exec(
      line,
    );
  if (match == null) {
    return null;
  ...
  return {
    ...
    ...(repeated && OPTIONAL_REPEATED_FIELDS.has(`${messageName}.${name}`)
...
function messageTypeExpression(name, parsed) {
  ...
  if (hasSingleOneofGroup(desc)) {
    return (
      desc.fields.map((field) => tsFieldType(field, parsed)).join(" | ") ||
      "Record<string, never>"
    );
  }
  ...
  for (const field of desc.fields) {
    ...
    lines.push(
      `  ${JSON.stringify(field.name)}${optional}: ${tsFieldType(field, parsed)};`,
    );
  }
  lines.push("}");
  return lines.join("\n");
...
function hasSingleOneofGroup(desc) {
  const group = desc.fields[0]?.oneofGroup;
  return (
    group != null && desc.fields.every((field) => field.oneofGroup === group)
  );
}

function singleValueTypeField(desc) {
  return desc.fields.length === 1 &&
    desc.fields[0]?.name === "value" &&
    desc.fields[0]?.oneof !== true
    ? desc.fields[0]
    : null;
}
